 ...
 * Features: Correct 'dw_totalcost' mapping and high-fidelity receipt previews.
 */
const DettaglioFullView: React.FC<DettaglioFullViewProps> = ({ 
  ...
  // Helper to extract formatted values
  ...
  const receipt = (detail as any).dw_receipt;
  const isPdf = receipt?.mimeType?.includes('pdf');
  const isImage = receipt?.mimeType?.includes('image');
    ...
    <div className="min-h-screen bg-slate-50/50 font-sans text-slate-900 pb-20">
      ...
      <main className="p-10 max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-10">
        ...
        <div className="lg:col-span-5 space-y-8">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-2xl font-black text-slate-900">
                      {getFormattedValue(detail, 'dw_currency')} {(detail as any).dw_totalcost?.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                    </p>
                  </div>
                </div>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-slate-50 p-5 rounded-xl border border-slate-100">
                <p className="text-sm text-slate-600 leading-relaxed italic">
                  {(detail as any).dw_additionalnotes || 'Nessuna nota aggiuntiva fornita per questa voce di spesa.'}
                </p>
              </div>
            </div>
          ...
          {/* Quick Actions / Audit Trail placeholder */}
          <div className="bg-slate-900 p-8 rounded-2xl text-white shadow-xl shadow-slate-200">
            <h4 className="font-bold mb-4 flex items-center gap-2">
              <FileText size={20} className="text-[#E85C24]" /> Supporto Decisionale
            </h4>
            <p className="text-sm text-slate-400 mb-6 leading-relaxed">
              Verifica che l'importo corrisponda esattamente alla ricevuta allegata e che la categoria sia coerente con la policy aziendale AGIC Group.
            </p>
        ...
        <div className="lg:col-span-7 space-y-6">
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col h-full min-h-[600px]">
              ...
              <div className="flex items-center gap-2">
                {receipt?.url && (
                  <>
                    ...
                    <button className="p-2 hover:bg-slate-100 rounded-lg text-slate-400 hover:text-[#E85C24] transition-all" title="Scarica">
                      <Download size={18} />
                    </button>
                  </>
                )}
              ...
              {isImage ? (
                ...
                <div className="w-full h-full flex flex-col items-center justify-center gap-6">
                  ...
                  <div className="text-center space-y-2">
                    <p className="font-bold text-slate-800">{receipt.fileName || 'documento_ricevuta.pdf'}</p>
                    <p className="text-xs text-slate-500">Visualizzazione anteprima PDF non disponibile nel browser.</p>
                  </div>
                  <a 
                    href={receipt.url} 
                    target="_blank" 
                    rel="noreferrer"
                    className="px-8 py-3 bg-white border border-slate-200 text-slate-700 font-bold rounded-xl shadow-sm hover:border-[#E85C24] hover:text-[#E85C24] transition-all flex items-center gap-2"
                  >
                    Visualizza PDF <ExternalLink size={18} />
                  </a>
                </div>
              ) : (
            ...
            <div className="px-8 py-4 bg-slate-50 border-t border-slate-100 text-[10px] text-slate-400 font-bold uppercase tracking-widest flex justify-between">
              <span>MIME-TYPE: {receipt?.mimeType || 'UNKNOWN'}</span>
              <span>DIMENSIONE: {receipt?.size ? `${(receipt.size / 1024).toFixed(1)} KB` : 'N/A'}</span>
            </div>
          </div>
        </div>
      </main>
