...
/**
 * AGIC Group Analytics & Spending Insights
 * Integrates live Dataverse data for total spend, trends, and category distribution.
 */
...
const AnalyticsPage: React.FC<AnalyticsPageProps> = ({ onNavigate }) => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      setRawExpenses((expensesRes as any)?.data ?? (expensesRes as any)?.value ?? []);
      setRawDetails((detailsRes as any)?.data ?? (detailsRes as any)?.value ?? []);
    } catch (err) {
  ...
  return rawDetails.filter((detail: any) => {
    ...
    const project =
      parentNota?.["_dw_codicedicommessa_value@OData.Community.Display.V1.FormattedValue"] ??
      parentNota?.["dw_codicedicommessa@OData.Community.Display.V1.FormattedValue"] ??
      "";

    const manager =
      parentNota?.["_dw_dipendente_value@OData.Community.Display.V1.FormattedValue"] ??
      "";
...
const timeFilteredDetails = useMemo(() => {
  ...
  const stats = useMemo(() => {
    if (!timeFilteredDetails.length) return { total: 0, avg: 0, pending: 0, count: 0 };
    
    const total = timeFilteredDetails.reduce((sum, d) => sum + (d.dw_totalcost ?? d.dw_amount ?? 0), 0);
...
const avg = count > 0 ? total / count : 0;
    ...
    // Calculate pending from header statuses (assuming 'dw_stato' code for 'In Attesa')
    const pendingTotal = rawExpenses
      .filter(e => e['dw_stato@OData.Community.Display.V1.FormattedValue']?.toUpperCase().includes('ATTESA'))
      .reduce((sum, e) => {
        // Find all details for this header to get cost
        const details = timeFilteredDetails.filter(d => d._dw_notaspesa_value === e.dw_nota_speseid);
        return sum + details.reduce((s, d) => s + (d.dw_totalcost ?? d.dw_amount ?? 0), 0);
      }, 0);

    return { total, avg, pending: pendingTotal, count };
  }, [rawExpenses, timeFilteredDetails]);

  // 2. Data for Monthly Trends
const trendData = useMemo(() => {
  const months = ["Gen", "Feb", "Mar", "Apr", "Mag", "Giu"];

  return months.map((month, index) => {
    const total = timeFilteredDetails
      .filter((detail: any) => {
        const createdOn = new Date(detail.createdon);
    ...
    const count = timeFilteredDetails.filter((detail: any) => {
      const createdOn = new Date(detail.createdon);
      return createdOn.getMonth() === index;
    }).length;
  ...
  // 3. Data for Category Distribution
  const categoryData = useMemo(() => {
    const categories: Record<string, number> = {};
    timeFilteredDetails.forEach(d => {
      const cat = d['dw_categoriadispesa@OData.Community.Display.V1.FormattedValue'] || 'Altro';
      categories[cat] = (categories[cat] || 0) + (d.dw_totalcost ?? d.dw_amount ?? 0);
    });
...
const projectData = useMemo(() => {
  ...
  timeFilteredDetails.forEach((detail: any) => {
    ...
    const project =
      parentNota?.["_dw_codicedicommessa_value@OData.Community.Display.V1.FormattedValue"] ??
      parentNota?.["dw_codicedicommessa@OData.Community.Display.V1.FormattedValue"] ??
      detail["_dw_codicedicommessa_value@OData.Community.Display.V1.FormattedValue"] ??
      "Senza commessa";
      ...
      <div className="space-y-8 animate-in fade-in duration-500">
        ...
        <div className="flex items-center justify-between bg-white p-8 rounded-2xl border border-slate-200 shadow-sm">
           <div className="space-y-1">
              <h2 className="text-sm font-black text-slate-400 uppercase tracking-widest">Spesa Totale</h2>
              <div className="flex items-center gap-3">
                 <p className="text-4xl font-black text-slate-900">€ {stats.total.toLocaleString(undefined, { minimumFractionDigits: 2 })}</p>
                 <span className="flex items-center gap-1 text-slate-500 text-sm font-bold bg-slate-50 px-2 py-0.5 rounded-lg border border-slate-100">
  Live Dataverse
</span>
              </div>
           </div>
           ...
           <div className="space-y-1 text-right lg:text-left">
              <h2 className="text-sm font-black text-slate-400 uppercase tracking-widest text-right">In Attesa</h2>
              <p className="text-2xl font-black text-orange-500">€ {stats.pending.toLocaleString()}</p>
           </div>
        </div>

        {/* Global Filters */}
        <section className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col lg:flex-row lg:items-center gap-6">
          <div className="flex items-center gap-4">
            <div className="relative">
              <Calendar className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={16} />
              <select 
                className="pl-10 pr-8 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold text-slate-600 outline-none hover:bg-slate-100 transition-all cursor-pointer"
                value={timeRange}
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {[
            { label: 'Costo Medio Nota', val: `€ ${stats.avg.toFixed(0)}`, sub: 'Calcolato dai dettagli', icon: DollarSign, color: 'text-blue-500', bg: 'bg-blue-50' },
            { label: 'Note Processate', val: stats.count, sub: 'Note Totali', icon: BarChart3, color: 'text-green-500', bg: 'bg-green-50' },
            { label: 'Categoria Top', val: categoryData[0]?.name || 'N/A', sub: 'Per Importo', icon: TrendingUp, color: 'text-orange-500', bg: 'bg-orange-50' },
            {
  label: "Urgenti",
  val: urgentCount,
  ...
  bg: "bg-red-50",
            ...
            <div key={i} className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex items-center justify-between">
              ...
              <div className={`w-12 h-12 rounded-xl ${m.bg} ${m.color} flex items-center justify-center shadow-inner`}>
                <m.icon size={24} />
              </div>
            </div>
          ))}
        ...
        <section className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 bg-white p-8 rounded-3xl border border-slate-200 shadow-sm space-y-6">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-lg font-black text-slate-800">Trend di Spesa Semestrale</h3>
                <p className="text-xs text-slate-400 font-medium uppercase tracking-tight">Analisi dei costi mensili consolidati</p>
              </div>
              <button className="p-2 hover:bg-slate-50 rounded-lg text-slate-400 transition-all"><BarChart3 size={18} /></button>
            </div>
            ...
            <div className="h-80 w-full">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={trendData}>
                  ...
                  />
                  <Area type="monotone" dataKey="total" stroke="#E85C24" strokeWidth={4} fillOpacity={1} fill="url(#colorTotal)" />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </div>
          ...
          <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm space-y-6 flex flex-col">
            <div className="space-y-1">
              <h3 className="text-lg font-black text-slate-800">Distribuzione Categorie</h3>
              <p className="text-xs text-slate-400 font-medium uppercase tracking-tight">Breakdown per tipologia di spesa</p>
            </div>
              ...
              {categoryData.slice(0, 3).map((c, i) => (
                <div key={i} className="flex items-center justify-between">
                  ...
                  <span className="text-xs font-black text-slate-900">
                    {stats.total > 0 ? ((c.value / stats.total) * 100).toFixed(0) : "0"}%
                  </span>
                </div>
              ))}
            </div>
                ...
                {projectData.slice(0, 10).map((row, i) => (
                  <tr key={i} className="hover:bg-slate-50/30 transition-colors group cursor-pointer">
                    <td className="px-8 py-6">
                       <span className="text-sm font-black text-[#E85C24] group-hover:underline underline-offset-4">{row.id}</span>
                    </td>
                    <td className="px-6 py-6 text-sm font-bold text-slate-600">
  {row.count} dettagli
</td>
                    <td className="px-6 py-6 text-sm font-black text-slate-900 text-right">
  € {row.total.toLocaleString(undefined, { minimumFractionDigits: 2 })}
</td>
                    <td className="px-6 py-6">
                       <span className="px-3 py-1 rounded-lg text-[9px] font-black uppercase bg-slate-50 text-slate-600 border border-slate-100">
